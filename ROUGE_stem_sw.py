# Вычисление метрики rouge с учетом стемминга и удалением стоп-слов
from Preprocessing import word_tokenizer, stop_words
from numpy import array
from nltk.stem.snowball import RussianStemmer


def _split_into_words(text):  # Разбиение на слова с учетом стемминга и удалением стоп-слов
    """
    ...
    """
    full_text_words = []
    full_text_words.extend(RussianStemmer().stem(word) for word in word_tokenizer.tokenize(text.lower()) if word not in stop_words)
...
def rouge_n_sw(evaluated, reference, n=2):
    """
    Вычисление ROUGE-N.
...
def rouge_l_sw(evaluated, reference):
    """
    Вычисляет ROUGE_L.
...
def rouge_s_sw(evaluated, reference, k):
    """
    Вычисляет ROUGE-S.
    Источник: ROUGE_paper.
    :param evaluated: Реферат, полученный алгоритмом.
